...
public class ExpressionSimplifier 
{
	...
	private static void replaceCellReferencesWithValue(ArrayList<String> items, Spreadsheet sheet) { 
		...
		for(int i = 0; i < items.size(); i++) {
			...
			if(sheet.isCellReference(token)) { //check if token is a cell reference
				...
				Cell touse = sheet.getData()[row][col]; //get the cell

				if(touse instanceof Numeric) { // get the numeric value of the cell
					numericvalue = ((Numeric) sheet.getData()[row][col]).getNumericValue();
				}
	...
	private static String calculateSum (Spreadsheet sheet, String cellReferenceStart, String cellReferenceEnd) {
		...
		for(int i = startRow; i <= endRow; i++) {
			for(int j = startCol; j <= endCol; j++) {
				if(sheet.getData()[i][j] == null) { //if a cell value is null, then set that cell value to zero
					Cell cell = new NumberCell("0");
					sheet.setData(i, j, cell);
				}
				sum += ((Numeric) sheet.getData()[i][j]).getNumericValue();
			}
		}
	...
	public static void main(String[] args) 
	{
		...
		sheet.setData(0, 0, cell);
		Cell cell1 = new NumberCell("2");
		sheet.setData(1, 0, cell1);
		Cell cell2 = new NumberCell("3");
		sheet.setData(2, 0, cell2);
		Cell cell3 = new NumberCell("4");
		sheet.setData(3, 0, cell3);
		Cell cell4 = new NumberCell("5");
		sheet.setData(4, 0, cell4);
